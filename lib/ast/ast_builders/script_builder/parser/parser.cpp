    ...
    while (EqualAtLeast(lexer_.PeekToken().type, TokenType::Assign, TokenType::PlusAssign, 
        ...
        if (op.type == TokenType::Assign) {
            ...
            switch (op.type) {
                ...
                default: throw parsing_error("Invalid operation type.", op.position);
            }
...
ExprNode Parser::ParsePrimary() {
    ...
    switch (token.type) {
        ...
        case TokenType::Number: {
            lexer_.NextToken();
            return ExprNode(new NumberExpr(static_cast<Value>(std::strtod(token.value.c_str(), nullptr)), token.position));
        }
        ...
        case TokenType::False: {
            lexer_.NextToken();
            return ExprNode(new NumberExpr(token.type == TokenType::True ? 1 : 0, token.position));
        }
...
ExprNode Parser::ParsePostfix(ExprNode&& var_node) {
    ...
    case TokenType::LParen: {
        ...
        Pos pos = lexer_.PeekToken().position;
        if (auto* var = dynamic_cast<VariableExpr*>(lhs.get())) {
            auto it = builtins_.find(var->var_name);
            if (it != builtins_.end()) {
                return ParsePostfix(ExprNode(new CallableExpr(it->second, std::move(args), pos)));
            }
        }

        return ParsePostfix(ExprNode(new CallableExpr(std::move(lhs), std::move(args), pos)));
    }
